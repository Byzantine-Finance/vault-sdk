...
export class ByzantineFactoryClient {
   ...
   * @param params The vault parameters
   * @param options Additional options for transaction (optional)
   * @returns Transaction receipt
   */
  async createSymbioticERC20Vault(
    params: SymbioticVault,
    options?: { populateOnly?: boolean }
  ) {
    if (!this.signer) {
      throw new Error("Signer is required for this operation");
    }
    ...
    try {
      ...
      // Call the contract method with explicit function signature to avoid ambiguity
      // Use the specific signature for the ERC20 version of createSymByzVault
      ...
      if (!functionFragment) {
        throw new Error("Function signature not found in contract ABI");
      } else {
        console.log("Function signature found in contract ABI");
      }

      // Add explicit transaction options with gas limit
      const overrides = {
        gasLimit: Math.floor(recommendedGasLimit),
      };

      console.log("Using gas limit:", overrides.gasLimit);

      // Get direct instance to avoid ethers validation
      const signer = this.signer;

      // Create a custom contract with the direct signer
      const contractWithSigner = this.contract.connect(signer);

      // If populateOnly is true, just return the populated transaction without sending it
      if (options?.populateOnly) {
        console.log("Populating transaction only (will not be sent)");

        // Use contract interface to encode function data
        const data = this.contract.interface.encodeFunctionData(
          functionFragment,
          [formattedBaseParams, formattedSymbioticParams]
        );
      ...
      // Manually call the function to bypass ethers gas estimations
      const tx = await contractWithSigner[
        functionFragment.format() as keyof typeof contractWithSigner
      ](formattedBaseParams, formattedSymbioticParams, overrides);
      ...
      if (error.code === "CALL_EXCEPTION") {
        console.error("Contract call reverted. This could be due to:");
        console.error("- Token not approved for the contract");
        console.error("- Incorrect parameter format");
        console.error("- Contract function parameter mismatch");
        console.error("- Contract constraints not met");
        ...
        if (error.error.message.includes("gas limit")) {
          console.error("Gas limit exceeded. Try with a smaller gas limit.");
          console.error(
            "You may need to modify the GAS_LIMITS constant in src/constants/index.ts"
          );
        }
      }
      ...
      console.error(
        ...
        Object.keys(this.contract.interface)
          .filter((f) => f.startsWith("createSymByzVault"))
          .join("\n")
      );
